...
- **Restore candidate v1**: Layer 1 restart discovery combines SessionRegistry pane IDs, assignment metadata, transcript ring summaries, Context Capsule refs, and Checkpoint refs. It is enumerate-only and does not perform automatic restore. Panes that can no longer be resumed, including expired agent sessions, are exposed as `setup-required` with a reason instead of being shown as ready or candidate. It does not store raw terminal transcripts, prompt bodies, secrets, or private local paths.
