...
| Gemini | 公式エージェント CLI 経由の Google OAuth | 互換目的 / tier 制限あり、この PC のみ |
...
- `gemini-api-key` は許可
- `gemini-vertex` は許可
- `antigravity-official-cli` はその PC 上での対話利用のみ
- `gemini-google-oauth` は 2026-06-18 以降は互換目的 / tier 制限付きで、上流 tier が提供を継続している場合に限り、その PC 上での対話利用のみ
