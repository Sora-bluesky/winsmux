...
| Antigravity CLI | 公式 `agy` CLI とセキュアキーチェーン経由の Google Sign-In | 当該 PC での対話利用のみ |
| Antigravity CLI | 公式 `agy` CLI 経由の企業向け Google アカウント / GCP オンボーディング | 正式サポート |
| Gemini | 公式エージェント CLI 経由の Gemini API key | 正式サポート |
| Gemini | 公式エージェント CLI 経由の Vertex AI の Gemini API | 正式サポート |
| Gemini CLI | 公式エージェント CLI 経由の Google OAuth | 従来のローカル利用。Antigravity CLI への移行対象 |
...
- `gemini-api-key` は許可
- `gemini-vertex` は許可
- `gemini-google-oauth` は従来のローカル利用で、`antigravity-google-signin` へ移行します
- `antigravity-google-signin` はその PC 上での対話利用のみ
- `antigravity-enterprise-gcp` は、公式 `agy` CLI が企業向けサインインを所有する場合に許可
