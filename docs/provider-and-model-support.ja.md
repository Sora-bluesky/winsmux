...
## Local Router Shadow Mode のモデルカード

この成果物の `artifact_id` は `winsmux-local-router-shadow-v03621`、
`policy_revision` は `v03621` です。これらは成果物とその方針を識別する値であり、
製品の対象バージョン `v0.36.38` ではありません。由来は、オフラインの Shadow Mode 評価用に
リポジトリで作成した、決定論的なルートヘッドのフィクスチャです。

この成果物はルート状態の特徴量を12個持ちます。重み付きのスロットヘッドとロールヘッドは
ロジットを出し、`local-router-shadow.ps1` が softmax で確率へ変換します。
`PowerShell 7` 上で `shadow_only` として動作し、出力は提案に限られます。実行権限は
決定論的ルーティングに残ります。必要な配布・信頼性の証跡は、
[Local Router Shadow Mode operations](operator-playbook.md#v03638-local-router-shadow-mode-operations)
を参照してください。

| 区分 | このモデルカードで言える範囲 |
| --- | --- |
| 検証済み | 正確なベースのソース、マニフェスト、重み、実行済みの配布ゲート、実行済みの固定100ケース信頼性受領記録、正確なmainのワークフローで直接示した動作だけを、検証済みと記載できます。 |
| 実験的 | 重み、しきい値、Shadow Mode の診断、オフラインのフィクスチャ結果は診断用の証拠だけです。一般的なモデル品質や本番利用の準備完了を示すものではありません。 |
| 未対応 | この成果物が本番ルーティングを変更する、プロバイダーを呼び出す、生のプロンプトを保存する、ファイルを書き込む、自律的なフェイルオーバーを行う、LLMまたはプロバイダーモデルである、実環境での品質・可用性・費用・安全性を証明する、と主張してはいけません。 |
...
| ------- | --------------- | -------------------------- |
| LM Studio | Windows x64 と ARM に対応し、x64 版は AVX2 が必須です。公式ドキュメントでは 16 GB RAM が推奨されています。OpenAI 互換 API をポート 1234 で提供し、Anthropic 互換 API（`/v1/messages`）も公式ドキュメント化されました。Anthropic 形式のクライアントは `ANTHROPIC_BASE_URL` をローカルサーバーに向けられます。 | 将来のローカルエンドポイント用アダプター候補です。現時点でも手動のローカルモデル検証に向きます。 |
| Ollama | Windows ネイティブアプリです。ダウンロードページの要件は Windows 10 以降です。現行の GPU ドキュメントでは NVIDIA は compute capability 5.0 以上とドライバー 550 以降が要件で、compute capability 5.0 から 6.2 では 570 以降が必要です。0.30 以降は llama.cpp 経由の GGUF 互換が広がり、Vulkan バックエンドが既定で有効です。ローカル API の既定バインドは `127.0.0.1:11434` のままです。 | 対応 GPU 経路がある PC では、将来のローカルエンドポイント用アダプター候補です。 |
...
| ONNX Runtime GenAI と Windows ML | ONNX 形式の生成モデルを Windows 上で動かす経路です。Windows ML の GenAI ライブラリは 2026-08 時点でも 0.x のプレビューとして説明されています。 | 将来のネイティブアプリ統合候補ですが、安定した既定バックエンドにはまだしません。 |
...
| 8 から 12 GB VRAM と 32 GB 以上の RAM の Windows PC | Q4 でおおむね 14B までの dense モデル（重み 7 から 9 GB 程度）、軽いコーディング補助、ローカルレビューの試行 | 個人向けでは最も現実的な層です。14B 級はこの層の 12 GB 側が前提です。active 3B 程度の 30B 級 MoE モデルも、RAM に余裕があれば CPU 部分オフロードで動きますが、速度は落ちます。 |
