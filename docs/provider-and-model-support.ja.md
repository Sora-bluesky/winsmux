...
| `api_llm` ワーカーバックエンド | OpenAI 互換の外部APIモデルワーカー用の契約として対応 | OpenRouter など、プロバイダー側が所有する API を使います。公開リポジトリでの標準導線は実行時の環境変数です。winsmux は伏せ字済みの実行メタデータだけを記録し、ローカル LLM や Colab LLM へ黙って切り替えません。 |
...
デスクトップの実行環境設定では、モデル候補を4種類に分けて表示します。
...
| `candidate` | worker backend と認証状態が一致する場合だけ割り当てます。たとえば OpenRouter は `api_llm` slot と実行環境の `WINSMUX_OPENROUTER_API_KEY` が必要です。 | local run id、遅延、費用、失敗理由、再現性が記録されるまでは候補証跡として表示します。 |
| `reference-only` | 比較文脈として表示しますが、デスクトップ picker では選択できません。 | Agent Arena、Code Arena、Colab モデル計画の参照値として表示できます。ただし winsmux がローカルで実行できるという意味にはしません。 |
...
winsmux は Fable 5 をベンチ参照行に残すことはできますが、公式に利用可能へ戻り、
provider/API 経路を再確認するまでは worker model として選択させません。

この利用可否判断は 2026-06-20 に Anthropic 公式の Fable ページと Claude app
release notes を再確認したものです。Fable 5 を `unavailable` から `selectable`
へ変える前に、同じ公式ページを再確認してください。

## 現行の Colab モデル対象

`v0.32.x` の Colab 対応は Google Colab 上での実行を対象にしており、ローカル LLM 提供は対象外です。
...
ドキュメント整備のための調査結果です。`v0.32.x` の Colab 対応の動作要件ではありません。
