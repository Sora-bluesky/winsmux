...
| ローカル LLM ランタイム | 今後のアダプター候補 | `v0.32.3` の動作要件にはしません。現時点では、通常のローカルツールとしてペインで動かすか、ローカルエンドポイントを使えるエージェント CLI 経由で使います。 |
...
`v0.32.3` の対象は Google Colab 上での実行であり、ローカル LLM 提供ではありません。
...
ドキュメント整備のための調査結果です。`v0.32.3` の動作要件ではありません。
