...
| `api_llm` ワーカーバックエンド | OpenAI 互換の外部APIモデルワーカー用の契約として対応 | OpenRouter など、プロバイダー側が所有する API を使います。API キーは実行時の環境変数、保管庫、またはローカル専用設定に置き、winsmux は伏せ字済みの実行メタデータだけを記録します。ローカル LLM や Colab LLM へ黙って切り替えません。 |
