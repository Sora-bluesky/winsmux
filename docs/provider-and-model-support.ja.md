...
大型モデルは、実ランタイムを開始する前に容量プリフライトを行います。たとえば GLM-5.2 系が350GiBの安全上限を超える場合、winsmux は Colab GPU を消費せずに停止し、`summary.json` に推定容量、失敗分類、次の行動を記録します。実行経路そのものを確認する場合は、容量確認を通ったモデルで先に adapter、Drive artifact、GUI表示を確認してください。
