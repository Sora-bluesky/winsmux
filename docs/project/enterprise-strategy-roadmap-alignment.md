...
This document describes how winsmux keeps non-public planning input aligned with the roadmap without turning that input into public product narrative.
...
| semantic context packs and durable coordination | `v1.3.0` | Describe managed coordination and context references. Do not expose non-public planning notes. |
...
| governance and threat-model packaging | `post-v1.0.0-governance` | Describe reviewable governance artifacts. Do not move local planning paths into public docs. |
...
- non-public input text
- maintainer-local planning paths
- prompt bodies or non-public skill bodies
- absolute market claims
- a new release lane created only from internal positioning

The public contract is the placement rule and the resulting durable surface. The non-public input that triggered the placement stays outside the public repository.
...
2. Public docs keep winsmux product language and do not reveal non-public input text.
...
5. The release note explains the public contract without exposing non-public inputs.
