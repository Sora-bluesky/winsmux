...
デスクトップアプリの自動更新検知は、`v0.36.23` のリリース条件です。Issue `#1082` として、利用可能な更新の検知、アプリ内の更新通知、インストーラーへの受け渡し、インストール進捗表示、再起動導線を実装します。`v0.36.23` は、この実装と検証が終わるまでリリースしません。`v0.36.23` より前の公開済みビルドは、新しいインストーラーを既存インストールの上から実行して更新します。
