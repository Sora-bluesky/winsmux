...
`isolated-enterprise` の実行では、実行単位のワークスペースを先に準備します。投影するファイルとディレクトリは、プロジェクト相対パスだけを指定します。

```powershell
winsmux workers workspace prepare w2 --include src --include docs/task.md --run-id run-123 --json
```

準備されたワークスペースは `.winsmux/isolated-workspaces/<slot>/<run>` に作られます。絶対パス、`..`、reparse point、予約名、依存ディレクトリ、ビルド成果物、秘密情報らしいファイルは拒否します。

実行後は同じ `run-id` で片付けます。
...
デスクトップアプリでは、View メニューからワーカーステータス表示を切り替えられます。非表示にしても状態そのものは `winsmux workers status --json` の契約に残り、Feed と通知はワーカー状態、ブロック中、オフライン、承認待ちへ接続します。
...
`local` で、通常の管理ペインの土台を保ちます。Colab GPU LLM の
デスクトップ E2E では、見える位置の `worker-1` と `worker-2` を
`colab_llm` に上書きします。トップレベルの `local` 既定を
モデルジョブの実行経路とは解釈しません。各スロットでは、次の値で上書きできます。

- `local`: 現在のローカル管理ペイン
- `codex`: Codex レビュー用、またはワーカー用のメタデータ
- `local_llm`: ローカルエンドポイント用ワーカーのメタデータと単発プロンプト実行
- `colab_cli`: `google-colab-cli` ワーカー用の状態メタデータ
- `colab_llm`: Colab GPU LLM モデルジョブのメタデータと単発実行
...
Colab MCP 用の別チェックアウトをアダプターとして使う場合は、同梱ブリッジを指定できます。
...
Colab Pro の GPU でモデルを動かす場合は、見えるワーカースロットに `colab_llm` を使います。
モデルファイルは Colab ランタイムが mount 済みの Google Drive cache へダウンロードします。
Windows PC や Ollama cache には保存しません。
...
ローカル LLM スロットは、Colab アダプターではなくローカルエンドポイントを使います。
初期対応ランタイムは Ollama です。モデルを pull する前に、`OLLAMA_MODELS` を
G ドライブ上の ASCII のみのモデルルートへ向けてください。Windows では
`G:\winsmux-local-llm\ollama-models` のようなパスを使います。ローカライズされた
フォルダー名など非 ASCII を含むモデルパスでは、`ollama list` が通っても
`llama-server` が model blob を開く段階で失敗することがあります。Google Drive の
ローカル表示名が非 ASCII の場合は、G ドライブの同期先へ戻るセッション用の
`subst` ドライブ、例えば `W:` を作り、`W:\winsmux-local-llm\ollama-models`
を使います。`artifact-root` または `WINSMUX_LOCAL_LLM_ARTIFACT_ROOT` も
G ドライブ上の成果物ルートへ向けます。winsmux が `.winsmux` に書くのは、
軽量な `run.json` と伏せ字済みの `stdout.log` だけです。

```powershell
winsmux workers doctor --json
winsmux workers status --json
winsmux workers exec worker-3 --prompt "このプロジェクトを要約してください。" --run-id local-smoke-1 --json
...
Colab GPU LLM デスクトップ E2E 用のスロット設定例です。この例では
意図的に `worker-1` と `worker-2` だけを示します。PC ローカルの
`local_llm` スロットは別の任意設定であり、この Colab レイアウトには含めません。
...
- Agent Vault で記録済みセッションを検索、絞り込み、再開する
- セッションカードをワーカーペインへドラッグして復元する
- 記録済みの作業を比較する
- ソースコード単位の詳細へ掘り下げる
...
デスクトップ終了時は、ウィンドウを閉じる場合も最終終了の場合も同じ片付け経路を通ります。サマリーストリームを止め、実行中のネイティブ音声キャプチャを止め、登録済みの PTY ペインをすべて取り出します。その後、ペインの子プロセスを終了させ、短時間だけ終了完了を待ちます。アプリ全体を閉じる前に 1 つのペインだけ閉じたい場合は、引き続き `pty.close` を使えます。
