...
この値はオペレーターの環境変数に置きます。1Password などのローカルな
パスワード管理ツールを使う場合も、起動前に環境変数へ入れる取得手段として扱います。
公開リポジトリの案内とリリース証跡では、環境変数を標準導線にしてください。
値そのものをリポジトリ、公開ドキュメント、ログ、リリース証跡へ書かないでください。
