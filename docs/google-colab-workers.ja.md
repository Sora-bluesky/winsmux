...
`v0.32.x` の Colab 対応で導入され、現在の `v0.36.x` リリースでも継続している機能として、winsmux は `google-colab-cli` 互換アダプターを通じて、
次の単発ワーカー操作を実行できます。
...
### 公式 Google Colab CLI 0.6.0

Google 公式の `google-colab-cli` パッケージは、現在 `colab` コマンドを
インストールします。`2026-06-17` 時点では、PyPI 上の最新版は
`google-colab-cli` `0.6.0` です。また、公式 README では Linux と macOS が
対応対象で、Windows は未対応とされています。

`WINSMUX_COLAB_CLI` に `colab` を直接指定しないでください。指定する場合は、
下記の winsmux アダプター契約を実装するラッパーを用意してください。公式の
`colab` コマンドは `colab exec`、`colab log`、`colab drivemount` などを
使います。一方、winsmux の `colab_cli` ワーカーは、実行証跡を安定した形で
記録するために、`run`、`logs`、`upload`、`download` を持つ互換レイヤーを
呼び出します。
...
- 想定アクセラレーターは `H100`、代替として `A100` を許容します。
...
アクセラレーター対象は `H100`、代替として `A100` を許容します。
...
停止コマンドも使ってください。
デスクトップ終了時は、そのデスクトップセッションが作成したローカルの PTY ワーカーペインを片付けます。リモートの Colab ノートブックやアダプター管理のランタイムはデスクトッププロセスツリーの外側にあるため、クォータ漏れを避ける必要がある場合は、アダプターとランタイムの停止を別途確認してください。

## 制限

Colab ワーカーレーンは、対話型の Colab REPL や console ループを自動化しません。
...
| 症状 | 確認すること |
| ------- | ----- |
| `google-colab-cli compatible adapter not found on PATH` | アダプターをインストールするか、`WINSMUX_COLAB_CLI` を設定します。 |
| 公式 `colab` CLI は見つかるがワーカーが縮退する | `WINSMUX_COLAB_CLI` に `scripts/google-colab-cli-adapter.ps1`、または公式CLIをwinsmuxアダプター契約へ変換するラッパーを指定します。 |
