...
このモック確認では、6 ワーカー構成、`workers status`、`workers doctor`、
単発実行、ローカルログとアダプター経由ログ、ディレクトリアップロードの除外、
ダウンロード、attach の挙動を確認します。偽アダプターは `new`、`status`、
...
実タスクを動かす前に、Colab 側の割り当てと停止方針を winsmux の外で確認してください。
...
| モック受け入れ確認は通るが実ランタイムが失敗する | 実プロジェクトで `workers doctor` を再実行し、アダプターのサインイン、ランタイムの割り当て、`WINSMUX_COLAB_ACCEPTANCE_REAL=1` の手動確認を順に見直します。 |
