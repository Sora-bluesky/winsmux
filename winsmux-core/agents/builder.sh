...
- Do NOT run git add, git commit, git push, or any git write commands unless a later explicit declarative managed completion instruction grants a task-scoped local git add/git commit exception for this WORKTREE.
- That explicit exception never grants git push or Git writes outside the assigned WORKTREE; follow its intended-file, hook, commit-message, and clean-status requirements exactly.
