...
fn color_idx7_and_idx15_are_not_swapped() {
    ...
    assert_ne!(
        idx7, idx15,
        "Idx(7) and Idx(15) must map to different Color variants"
    );
...
fn color_rgb_passthrough() {
    assert_eq!(
        vt_to_color(vt100::Color::Rgb(255, 128, 0)),
        Color::Rgb(255, 128, 0)
    );
}
...
fn hidden_cell_renders_as_space() {
    ...
    for col in 0..6 {
        ...
        assert_eq!(
            text, " ",
            "Hidden cell at col {col} must render as space, got {:?}",
            text
        );
    }
}
...
fn strikethrough_cell_has_crossed_out_modifier() {
    ...
    assert!(
        ...
        "Strikethrough cell style must contain CROSSED_OUT modifier, got: {:?}",
        style
    );
}
...
fn strikethrough_cell_still_renders_text() {
    ...
    assert_eq!(
        text, "X",
        "Strikethrough cell should render actual content, not space"
    );
}
...
fn hidden_with_bold_still_renders_as_space() {
    ...
    assert_eq!(
        text, " ",
        "Hidden overrides content regardless of other modifiers"
    );
    assert!(
...
fn strikethrough_hidden_cell_renders_as_space_with_crossed_out() {
    ...
    assert_eq!(
        text, " ",
        "Hidden cell renders as space even with strikethrough"
    );
    assert!(
        style.add_modifier.contains(Modifier::CROSSED_OUT),
...
fn ratatui_buffer_hidden_cell_produces_space_in_output() {
    ...
    for col in 0..6u16 {
        assert_eq!(
            buf[(col, 0u16)].symbol(),
            " ",
            "Buffer cell at col {col} (hidden) should be space, got {:?}",
            buf[(col, 0u16)].symbol()
        );
    }
...
fn ratatui_buffer_strikethrough_has_correct_modifier() {
    ...
    for col in 0..6u16 {
        ...
        assert!(
            bcell.modifier.contains(Modifier::CROSSED_OUT),
...
fn ratatui_buffer_color_idx7_is_gray() {
    ...
    assert_eq!(
        buf[(0u16, 0u16)].fg,
        Color::Gray,
        "SGR 37 (index 7) in buffer must be Color::Gray, not {:?}",
        buf[(0u16, 0u16)].fg
    );
}
...
fn ratatui_buffer_color_idx15_is_white() {
    ...
    assert_eq!(
        buf[(0u16, 0u16)].fg,
        Color::White,
        "SGR 97 (index 15) in buffer must be Color::White, not {:?}",
        buf[(0u16, 0u16)].fg
    );
}
...
fn crossterm_output_strikethrough_emits_sgr9() {
    ...
    let cells: Vec<(u16, u16, &ratatui::buffer::Cell)> = buf
        .content()
        .iter()
        .enumerate()
        .map(|(i, cell)| {
...
fn crossterm_output_hidden_cell_is_space_not_sgr8() {
    ...
    let visible: String = out_str
        ...
        .collect();
    assert!(
        visible.contains("DEF"),
        "Visible output should contain 'DEF'. Full text: {:?}",
        visible
    );
}
