...
fn machine_contract_exposes_version_and_canonical_roles() {
    ...
    assert_eq!(catalog.version, "0.36.28");
    assert_eq!(
        catalog
            .roles
            .iter()
...
fn machine_contract_exposes_worker_backend_contracts() {
    ...
    assert_eq!(
        catalog
            ...
            .collect::<Vec<_>>(),
        vec!["local", "codex", "api_llm", "antigravity", "noop"]
    );
    ...
    let antigravity = catalog
        .worker_backends
        .iter()
        .find(|backend| backend.id == "antigravity")
        .expect("antigravity backend should exist");
    assert!(antigravity.runtime_available);
    assert!(antigravity.config_fields.contains(&"agent"));
    assert!(antigravity.config_fields.contains(&"model"));
    assert!(antigravity.config_fields.contains(&"model_source"));
    assert!(antigravity.config_fields.contains(&"prompt_transport"));
    assert!(antigravity.config_fields.contains(&"auth_mode"));
...
fn machine_contract_serializes_to_json() {
    ...
    assert_eq!(value["version"], "0.36.28");
    ...
    assert_eq!(value["worker_backends"][2]["id"], "api_llm");
    assert_eq!(value["worker_backends"][2]["runtime_available"], true);
    assert_eq!(value["worker_backends"][3]["id"], "antigravity");
