...
fn normalize_manifest_pane(
    ...
    NormalizedManifestPane {
        ...
        write_scope: pane.write_scope.values(),
        read_scope: pane.read_scope.values(),
        constraints: pane.constraints.values(),
        expected_output: pane.expected_output.clone(),
        verification_plan: pane.verification_plan.values(),
        review_required: pane.review_required.value().unwrap_or(false),
        provider_target: pane.provider_target.clone(),
        agent_role: pane.agent_role.clone(),
        timeout_policy: pane.timeout_policy.clone(),
        handoff_refs: pane.handoff_refs.values(),
