...
pub fn run_status_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::status_payload(&snapshot, &options.project_dir);
    write_json(&payload)
}
...
pub fn run_board_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::board_payload(&snapshot, &options.project_dir);
    if options.json {
        return write_json(&payload);
    }
    let payload = read_path::payload_to_value(&payload)?;
...
pub fn run_inbox_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::inbox_payload(&snapshot, &options.project_dir);
    if options.json {
        write_json(&payload)
    } else {
        let payload = read_path::payload_to_value(&payload)?;
...
pub fn run_digest_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::digest_payload(&snapshot, &options.project_dir);
    if options.json {
        write_json(&payload)
    } else {
        let payload = read_path::payload_to_value(&payload)?;
...
pub fn run_desktop_summary_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::desktop_summary_payload(&snapshot, &options.project_dir)?;
    if options.json {
        return write_json(&payload);
    }
...
pub fn run_runs_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::runs_payload(&snapshot, &options.project_dir);
    if options.json {
        write_json(&payload)
    } else {
        let payload = read_path::payload_to_value(&payload)?;
...
pub fn run_explain_command(args: &[&String]) -> io::Result<()> {
    ...
    let snapshot = read_path::load_snapshot(&options.project_dir)?;
    let payload = read_path::explain_payload(&snapshot, &options.project_dir, &run_id)?;
    if options.follow {
        if options.json {
            write_json(&payload)?;
        } else {
            let payload_value = read_path::payload_to_value(&payload)?;
...
fn run_compare_runs_with_usage(command_name: &str, args: &[&String]) -> io::Result<()> {
    ...
    let (left, right) = read_path::compare_run_projections(&snapshot, &left_id, &right_id)?;
...
fn run_promote_tactic_with_usage(command_name: &'static str, args: &[&String]) -> io::Result<()> {
    ...
    let consultation_packet = read_path::read_artifact_json(
...
const EVENTS_FOLLOW_POLL: Duration = Duration::from_millis(100);
...
pub fn run_events_command(args: &[&String]) -> io::Result<()> {
    ...
    let (lines, _) = read_events_jsonl_lines(&options.project_dir, false)?;
    let mut events = Vec::new();
    for (index, line) in lines.iter().enumerate().skip(options.cursor) {
        match map_complete_event_line(line, index)? {
...
fn follow_condensed_events(options: &EventsOptions) -> io::Result<()> {
    ...
    loop {
        let (lines, _) = read_events_jsonl_lines(&options.project_dir, true)?;
        if lines.len() < cursor {
            cursor = 0;
        }
        for (index, line) in lines.iter().enumerate().skip(cursor) {
        ...
        thread::sleep(EVENTS_FOLLOW_POLL);
    }
}
...
fn read_events_jsonl_lines(
    ...
    let (body, torn) = if drop_torn_tail && !raw.ends_with('\n') {
        match raw.rfind('\n') {
            Some(idx) => (&raw[..=idx], true),
...
fn parse_events_options(args: &[&String]) -> io::Result<EventsOptions> {
    ...
    if let Some(raw_cursor) = positionals.first() {
        let parsed = raw_cursor.parse::<i32>().map_err(|_| {
            io::Error::new(io::ErrorKind::InvalidInput, usage_for("events"))
        })?;
        if parsed > 0 {
            cursor = parsed as usize;
        }
...
fn usage_for(command: &str) -> &'static str {
    match command {
        ...
        "events" => {
            "usage: winsmux events [cursor] [--follow] [--json] [--project-dir <path>]"
        },
...
fn consultation_command_context(
    ...
    if !run_id_override.trim().is_empty() {
        let snapshot = read_path::load_snapshot(project_dir)?;
...
fn wait_for_shell_prompt(pane_id: &str) -> io::Result<()> {
    ...
    while Instant::now() < deadline {
        ...
        if crate::shell_lifecycle::shell_prompt_ready(&text) {
            return Ok(());
        }
