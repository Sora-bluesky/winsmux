...
Claude Code、Codex CLI、Gemini CLI を 1 つずつ手で眺める段階を越えたい。ただし、クラウド任せにも、特定ベンダー任せにもしたくない。`winsmux` はそのためのローカル管制面です。
...
- 既定で 6 つの管理ワーカースロットを作成します。ローカル、Codex、Colab CLI、仮置きワーカー向けに、バックエンド契約と縮退状態の表示を用意しています。
