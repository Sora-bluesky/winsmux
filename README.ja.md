...
- `winsmux init` と `winsmux launch` は、公開向けの first-run 導線として使えます
...
現在公開しているクイックスタートは、`winsmux init` と `winsmux launch` を起点にする手順です。`winsmux compare` は今後追加予定の公開向けエントリポイントです。`/winsmux-start` は Claude Code による dogfooding 専用フローとして扱います。
...
compare surface が入るまでは、`winsmux conflict-preflight <left_ref> <right_ref>` を merge risk 確認の公開 CLI として使います。
