...
| `psmux-bridge read <target> [lines]` | ペインの末尾 N 行を読み取り（デフォルト 200） |
...
| `psmux-bridge name <target> <label>` | ペインにラベルを付ける（ボーダー表示を自動有効化） |
...
| `psmux-bridge focus <label\|target>` | アクティブペインを切り替え（psmux 外から使用） |
