...
- `winsmux compare` は、引き続き公開向けの主要導線へ寄せる予定です
...
現在公開しているクイックスタートは、`winsmux init` と `winsmux launch` を起点にする手順です。`winsmux compare` は今後追加予定の公開コマンドです。`/winsmux-start` は Claude Code による dogfooding 専用フローとして扱います。
...
`winsmux init` は、現在の project 向けに既定の `.winsmux.yaml` を作ります。
...
比較向けのペアテンプレートも表示します。
`winsmux launcher save <name>` と `winsmux launcher list` は、再利用する起動テンプレートを project の `.winsmux` ディレクトリに保存します。
比較画面が入るまでは、`winsmux conflict-preflight <left_ref> <right_ref>` を競合リスク確認の公開 CLI として使います。
...
| `winsmux launcher <presets|list|save>` | 起動プリセットとペアテンプレートを表示または保存 |
