...
現在の public quick start は、下記の terminal-first runtime flow です。将来の public entrypoint は `winsmux init`、`winsmux launch`、`winsmux compare` に収束させます。repo 内 contributor 向けの起動 helper はありますが、public interface には含めません。
