...
現在公開しているクイックスタートは、下記のターミナル操作が起点となる手順です。`winsmux init`、`winsmux launch`、`winsmux compare` は今後追加予定の公開向けエントリポイントです。`/winsmux-start` は Claude Code による dogfooding 専用フローとして扱います。
