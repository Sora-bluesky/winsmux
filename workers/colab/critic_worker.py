#!/usr/bin/env python3
"""Self-contained critic worker template for winsmux Colab slots."""
...
WORKER_KIND = "critic"
ARTIFACT_FILE = "critic-notes.md"
ACTIONS = ["review changed behavior", "classify findings by severity", "check verification evidence"]
RISKS = ["template requires a diff or artifact bundle to produce concrete findings"]
...
{bullet_list(commands, "Check whether the submitted evidence covers the behavior.")}

## Critic Checklist

- Identify correctness, safety, and user-visible behavior risks first.
...
def success_payload(args: argparse.Namespace, worker_id: str, run_id: str, task: dict[str, Any], artifact_path: Path) -> dict[str, Any]:
    return {
        ...
        "actions": ACTIONS,
        "artifacts": [
            {"kind": "markdown", "path": artifact_path.as_posix(), "description": "Critic review focus and checklist."}
        ],
        "observations": [],
        "risks": RISKS,
        "errors": [],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="winsmux Colab critic worker template")
