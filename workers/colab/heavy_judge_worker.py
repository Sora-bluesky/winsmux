#!/usr/bin/env python3
"""Self-contained heavy second-opinion worker template for winsmux Colab slots."""
...
WORKER_KIND = "heavy_judge"
ARTIFACT_FILE = "heavy-judge-report.md"
ACTIONS = ["re-check evidence", "identify release blockers", "separate follow-up recommendations"]
RISKS = ["template is a structured entrypoint; the selected model supplies the judgment"]
...
def render_artifact(task: dict[str, Any]) -> str:
    ...
    commands = unique_text_items(task, ("commands", "verification_plan", "tests"))
    return f"""# Heavy Judge Report Template
...
{bullet_list(files, "Use the review packet or attached artifacts as the evidence source.")}

### Commands

{bullet_list(commands, "Check whether validation evidence is sufficient.")}

## Judgment Rubric

- Blocking correctness or data-loss risk.
- Security or secret-handling risk.
- Release-blocking test or documentation gap.
...
def success_payload(args: argparse.Namespace, worker_id: str, run_id: str, task: dict[str, Any], artifact_path: Path) -> dict[str, Any]:
    return {
        ...
        "summary": "prepared a heavy second-opinion template",
        "actions": ACTIONS,
        "artifacts": [
            {"kind": "markdown", "path": artifact_path.as_posix(), "description": "Second-opinion judgment rubric."}
        ],
        "observations": [],
        "risks": RISKS,
        "errors": [],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="winsmux Colab heavy judge worker template")
