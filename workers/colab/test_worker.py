#!/usr/bin/env python3
"""Self-contained test worker template for winsmux Colab slots."""
...
WORKER_KIND = "test"
ARTIFACT_FILE = "test-plan.md"
ACTIONS = ["run focused tests", "capture exit codes", "return concise failure evidence"]
RISKS = ["template does not run commands automatically without an explicit executor"]
...
{bullet_list(commands, "Select the smallest test command that covers the changed behavior.")}

## Evidence To Return

- Command, exit code, and short result.
- Failing test names or error summaries.
- Any skipped coverage and the reason.
"""
...
def success_payload(args: argparse.Namespace, worker_id: str, run_id: str, task: dict[str, Any], artifact_path: Path) -> dict[str, Any]:
    return {
        ...
        "summary": "prepared a test worker template",
        "actions": ACTIONS,
        "artifacts": [
            {"kind": "markdown", "path": artifact_path.as_posix(), "description": "Test execution plan and evidence checklist."}
        ],
        "observations": [],
        "risks": RISKS,
        "errors": [],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="winsmux Colab test worker template")
