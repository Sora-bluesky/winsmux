...
function New-WinsmuxLocationIdentity {
    param(
        [Parameter(Mandatory = $true)][ValidateSet('local_file', 'local_directory', 'pane_log', 'source_ref')][string]$Kind,
...
function Get-WorkersStatusRows {
    ...
    foreach ($slot in @($Context.Slots)) {
        ...
        $state = $paneState
        if ($manifestStatus -in @('deferred_start', 'deferred_starting', 'deferred_start_failed', 'api_llm_runner_unconfigured', 'antigravity_runner_unconfigured')) {
        ...
        if ($null -ne $heartbeat) {
            ...
            if ($heartbeatCanDriveState -and
                ...
                $state -notin @('deferred_start_failed', 'deferred_start', 'deferred_starting', 'api_llm_runner_unconfigured', 'antigravity_runner_unconfigured')) {
...
function Get-WorkersInputMaxBytes {
    $raw = [string]$env:WINSMUX_WORKER_INPUT_MAX_BYTES
...
function Get-WorkersSafetyFinding {
    ...
    foreach ($value in @($Values)) {
        ...
        if ($text -match '(?is)-----BEGIN [A-Z0-9 ]*PRIVATE KEY-----.*?-----END [A-Z0-9 ]*PRIVATE KEY-----') {
            return [PSCustomObject]@{ Code = 'secret_like_input'; Source = 'worker_task_input' }
        }
        ...
        foreach ($rule in @($rules)) {
            if ($text -match [string]$rule.Pattern) {
                return [PSCustomObject]@{ Code = [string]$rule.Code; Source = 'worker_task_input' }
            }
        }
    }
...
function Assert-WorkersApiLlmSafetyInput {
    ...
    $finding = Get-WorkersSafetyFinding -Values $Values
    if ($null -ne $finding) {
...
function Get-WorkersExecSafetyInputValues {
    ...
    if (-not [string]::IsNullOrWhiteSpace($TaskJsonPath)) {
        $taskJsonInfo = Resolve-WorkersProjectPath -ProjectDir $ProjectDir -Path $TaskJsonPath -MustExist -AllowFile -MaxBytes (Get-WorkersInputMaxBytes)
...
function Invoke-WorkersApiLlmExec {
    ...
    $inputInfo = Resolve-WorkersProjectPath -ProjectDir $Options.ProjectDir -Path $inputPath -MustExist -AllowFile -AllowSubmissionPacket:($inputKind -eq 'task_json') -MaxBytes (Get-WorkersInputMaxBytes)
...
function Invoke-WorkersExec {
    $usage = "usage: winsmux workers exec <slot> [--script <path> | --task-json <path>] [--task-id <id>] [--run-id <id>] [--json] [--project-dir <path>]; supported backends: api_llm, antigravity"
    ...
    Stop-WithError "workers exec does not support backend '$($workerProbe.Row.Backend)'"
}
...
function Invoke-WorkersLogs {
    ...
    Stop-WithError "workers logs does not support backend '$($workerProbe.Row.Backend)'"
}
...
Commands:
  ...
  workers <exec|logs> <slot> ... [--json] [--project-dir <path>]  Execute or inspect api_llm and antigravity worker runs
