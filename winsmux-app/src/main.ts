...
const runtimeReasoningOptions: Array<{ value: RuntimeReasoningEffort; label: string; labelJa: string }> = [
  ...
  { value: "xhigh", label: "X High", labelJa: "非常に高い" },
];

const runtimeModelCatalog: RuntimeModelCatalogEntry[] = getRuntimeCatalogEntries().map((entry) => ({
  ...entry,
...
function getRuntimePaneModelEntries(provider: RuntimeProviderId): RuntimeModelCatalogEntry[] {
  ...
  if (provider === "grok-build" && !entries.has("grok-build")) {
    entries.set("grok-build", createRuntimeCatalogEntry({
      ...
      modelSource: "cli-discovery",
      reasoningEffort: "provider-default",
      supportedReasoningEfforts: ["provider-default"],
...
function getClaudeRuntimeReasoningProfile(model: ComposerModelId): { defaultEffort: RuntimeReasoningEffort; supportedEfforts: RuntimeReasoningEffort[] } {
  switch (model) {
    case "fable-5":
    case "opus-4.8":
      return { defaultEffort: "high", supportedEfforts: ["low", "medium", "high", "max", "xhigh"] };
    case "opus-4.7":
    case "opus-4.7-1m":
      return { defaultEffort: "xhigh", supportedEfforts: ["low", "medium", "high", "max", "xhigh"] };
    case "opus-4.6":
    case "sonnet-4.6":
