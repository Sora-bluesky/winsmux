...
Describe 'winsmux version surface' {
    ...
    It 'runs verify with the same Pester discovery boundary as CI' {
        ...
        $bridgeScript | Should -Match '\$pesterConfiguration\.Run\.Path = @\(\$testsDir\)'
        $bridgeScript | Should -Match '\$pesterConfiguration\.Run\.PassThru = \$true'
        $bridgeScript | Should -Match '\$pesterConfiguration\.Run\.Exit = \$false'
        $bridgeScript | Should -Match 'Invoke-Pester -Configuration \$pesterConfiguration'
